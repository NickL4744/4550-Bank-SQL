...
$Acct_no = '1234567890';  // Example account ID
...
try {
    ...
    $query = "SELECT balance FROM `checking` WHERE `Acct_no` = '$Acct_no'";
    ...
    // Add the withdrawal transaction to the checking transactions table
    $query = "INSERT INTO `checking_transactions` (`Acct_no`, `trans_type`, `amount`) VALUES ('$Acct_no', 'withdrawal', '$withdrawalAmount')";
    ...
    $query = "UPDATE `checking` SET `balance` = `balance` - $withdrawalAmount WHERE `Acct_no` = '$Acct_no'";
    if (!mysqli_query($dbcnx, $query)) {
