...
try {
    // Add the deposit transaction to the savings transactions table
    $query = "INSERT INTO `savings_transactions` (`accountID`, `transaction_type`, `amount`) VALUES ('$accountID', 'deposit', '$depositAmount')";
    ...
    $query = "UPDATE `savings` SET `balance` = `balance` + $depositAmount WHERE `accountID` = '$accountID'";
    if (!mysqli_query($dbcnx, $query)) {
