...
if ($currentDate === $firstOfMonth) {
    ...
    try {
        ...
        $query = "SELECT accountID, balance FROM `checking`";
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            // Add the interest transaction to the interest transactions table
            $query = "INSERT INTO `interest_transactions` (`accountID`, `transaction_type`, `amount`, `transaction_date`) 
            ...
            $query = "UPDATE `checking` SET `balance` = $newBalance WHERE `accountID` = '$accountID'";
            if (!mysqli_query($dbcnx, $query)) {
